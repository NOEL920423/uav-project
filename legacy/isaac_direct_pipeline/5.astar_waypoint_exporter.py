...
#
# No ROS, no camera, no OpenCV, no machine learning.
...
EXPORT_WAYPOINTS_ONLY = True
ROS2_WAYPOINT_JSON_PATH = "/home/noel_614420090/uav_ros2_ws/waypoints.json"
...
OBSTACLE_NAME_PREFIXES = ["Obstacle_", "obstacle_", "CylinderObstacle_"]
...
PATH_LOOKAHEAD_M = 0.90
MIN_LOOKAHEAD_M = 0.50
MAX_LOOKAHEAD_M = 1.30
...
def make_obstacle_info_from_prim(prim) -> Optional[ObstacleInfo]:
    ...
    estimated_radius = max(float(bbox_size[0]), float(bbox_size[1])) * 0.5
...
def main():
    ...
    print("")
    print("========== PX4 Ground Truth A* Lookahead Episode Runner ==========")
    print("[Main] This script does NOT generate scene objects.")
    ...
    if EXPORT_WAYPOINTS_ONLY:
        save_ros2_waypoints_json(
            output_path=ROS2_WAYPOINT_JSON_PATH,
            episode_id=episode_id,
            start_ned=start_ned,
            final_goal_ned=final_goal_ned,
            planning_result=planning_result,
        )
        print("[Main] EXPORT_WAYPOINTS_ONLY=True. Mission will NOT connect to PX4.")
        print("[Main] Next step: run ros2_waypoint_follower.py from terminal.")
        return
